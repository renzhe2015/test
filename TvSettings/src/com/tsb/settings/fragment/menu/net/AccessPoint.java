...
class AccessPoint extends Preference {
    ...
    static int getSecurity(WifiConfiguration config) {
        ...
        return (config.wepKeys[0] != null || config.wepKeys[1] != null || config.wepKeys[2] != null || config.wepKeys[3] != null) ? SECURITY_WEP : SECURITY_NONE;
    }
    ...
    protected void onBindView(View view) {
        ...
        ImageView mSignalSecurity = (ImageView) view.findViewById(R.id.signal_lock);
        if (mRssi == Integer.MAX_VALUE) {
        	mSignal.setImageDrawable(null);
        } else {
        	mSignal.setImageResource(R.drawable.tcl_wifi_signal_open);
        	mSignal.setImageLevel(getLevel());
        	mSignalSecurity.setImageResource(R.drawable.wifi_signal);
            mSignalSecurity.setImageState((security != SECURITY_NONE) ?
                    STATE_SECURED : STATE_NONE, true);
        }
        
        Log.d(TAG , "onBindView:mState===>" + mState);
        ImageView mConnectState = (ImageView) view.findViewById(R.id.connected_state);
    	if (mConnectState != null) {
    		if (mState == DetailedState.CONNECTED) {
    		mConnectState.setVisibility(View.VISIBLE);
    		} else {
    			mConnectState.setVisibility(View.GONE);
    		}
    	}
    }
